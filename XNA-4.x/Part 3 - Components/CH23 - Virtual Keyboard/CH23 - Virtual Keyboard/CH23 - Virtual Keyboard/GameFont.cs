...
namespace Codetopia.Graphics
{
    public class GameFont
    {
        ...
        //protected static readonly uint CHAR_MASK = 0x00FFFF00;
        ////protected static readonly uint MAGIC_MASK = 0xFFFF0000;
        ////protected static readonly uint MAGIC_NUMBER = 0xC0DE0000;
        //protected static readonly uint MAGIC_MASK = 0xFF0000FF;
        //protected static readonly uint MAGIC_NUMBER = 0xC00000DE;
        ...
        protected void ExtractGlyphDescriptors()
        {
            ...
            bool valid = w > 0 && h > 1 &&
                ...
                data[0].A == 0xC0 && data[0].R == 0xDE &&
                data[w].A == 0xC0 && data[w].R == 0xDE;

            // is this a valid font texture
            if (valid)
            {
                // record the height and ascent of this font
                _fontHeight = (int)(data[0].G * 256 + data[0].B);
                _fontAscent = (int)(data[w].G * 256 + data[w].B);
                ...
                for (int y = 0; y < h; y += _fontHeight + 1)
                {
                    ...
                    if ((data[y * w + nFirstColumn].A) == 0xFF)
                    {
                        // found a marker, scan the row for glyphs
                        for (int x = nFirstColumn; x < w; x++)
                        {
                            // is this a glyph?
                            if (data[y * w + x].A == 0xFF)
                            {
                                // yes. record the details and ...
                                char key = (char)(data[y * w + x].G * 256 + data[y * w + x].B);
                                int top = y + 1;
                                int left = x;
                                int width = 1;

                                // ... keep scanning to determine the width
                                while ((x + width < w - 1) && (data[y * w + x + width].A != 0xFF))
                                {
                                    width++;
                                }
